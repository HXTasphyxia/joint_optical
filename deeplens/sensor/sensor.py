...
class SpectralSensor(RGBSensor):
    def __init__(
            self,
            bit=10,
            black_level=64,
        ...
        # 初始化31波段的光谱响应函数
        self.wavelengths = wavelengths
        self.spectral_responses = spectral_responses

    def response_curve(self, img_spectral):
        """应用光谱响应曲线将31通道高光谱图像转换为RGB图像

        Args:
            img_spectral: 形状为[B, 31, H, W]的高光谱图像

        Returns:
            img_raw: 形状为[B, 3, H, W]的RGB图像
        """
        # 使用给定的sensor光谱响应函数进行处理
        img_raw = torch.zeros(
            (img_spectral.shape[0], 3, img_spectral.shape[2], img_spectral.shape[3]),
            device=img_spectral.device,
        )

        # 对每个RGB通道应用光谱响应
        for i in range(3):  # R, G, B
            response = self.spectral_responses[i]  # 获取该通道的31波段响应
            img_raw[:, i, :, :] = (img_spectral * response.view(1, -1, 1, 1)).sum(dim=1)
