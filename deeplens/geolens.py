...
class GeoLens(Lens):
    ...
    # Image simulation
    # ====================================================================================
    # def render_spectral(self, img_spectral, depth=DEPTH, method="raytracing", **kwargs):
    #     """Differential rendering for 31-channel hyperspectral images
    #
    #     Args:
    #         img_spectral: A hyperspectral image with the shape of [B, 31, H, W]
    #         depth: The depth of an object
    #         method: Rendering method
    #
    #     Returns:
    #         img_rgb: An RGB image with the shape of [B, 3, H, W]
    #     """
    #     # Set the resolution of the sensor
    #     if not (
    #             self.sensor_res[0] == img_spectral.shape[-2]
    #             and self.sensor_res[1] == img_spectral.shape[-1]
    #     ):
    #         H, W = img_spectral.shape[-2], img_spectral.shape[-1]
    #         self.set_sensor(sensor_res=(H, W), sensor_size=self.sensor_size)
    #
    #         # Optical rendering is performed separately for each wavelength
    #     if method == "raytracing":
    #         # Create PSFs respectively for the 31 channels
    #         psfs = []
    #         for wvln range(400, 710, 10):
    #             rays = self.sample_sensor(spp=kwargs.get("spp", SPP_RENDER), wvln=wvln)
    #             rays = self.trace2obj(rays, depth=depth)
    #             psfs.append(rays)
    #
    #             # Optical rendering is performed using a 31-channel PSF
    #         img_render_spectral = torch.zeros_like(img_spectral)
    #         for i in range(31):
    #             img_render_spectral[:, i, :, :] = self.render_raytracing_mono(
    #                 img=img_spectral[:, i, :, :],
    #                 wvln=0.4 + i * 0.01,
    #                 depth=depth,
    #                 ray=psfs[i],
    ...
    #             )
    #
    #             # Convert the hyperspectral image to an RGB image
    #         # img_rgb = self.spectral_sensor.response_curve(img_render_spectral)
    #
    #     return img_render_spectral
    ...
    def render(self, img_obj, depth=DEPTH, method="raytracing", **kwargs):
        ...
        if method == "psf_map":
            ...
            if "spp" in kwargs:
                ...
                img_render = self.render_raytracing_spectral(img_obj, depth=depth)
    ...
    def render_raytracing_spectral(self, img, depth=DEPTH, spp=SPP_RENDER, vignetting=False):
        """Render spectral image using ray tracing method.

        Args:
            img (tensor): spectral image tensor. Shape of [N, 31, H, W].
            depth (float, optional): Depth of the object. Defaults to DEPTH.
            spp (int, optional): Sample per pixel. Defaults to 64.
            vignetting (bool, optional): whether to consider vignetting effect. Defaults to False.

        Returns:
            img_render (tensor): Rendered spectral image tensor. Shape of [N, 31, H, W].
        """
        img_render = torch.zeros_like(img)
        for i in range(31):
            img_render[:, i, :, :] = self.render_raytracing_mono(
                img=img[:, i, :, :],
                wvln=WAVE_BOARD_BAND[i],
                depth=depth,
                spp=spp,
                vignetting=vignetting,
            )
